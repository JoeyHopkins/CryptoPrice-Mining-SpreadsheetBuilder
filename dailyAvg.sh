...
getCurrentPrice() {
	...
	curl -H "X-CMC_PRO_API_KEY: $APIKey" -H "Accept: application/json" -d "id=$APIID&convert=USD" -G https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest -o $CURRENTPRICE

        #Isolate the price in a text file
        sed -i 's/\,/\n/g' $CURRENTPRICE
        sed -i 's/{/\n/g' $CURRENTPRICE
        sed -i '/price/!d' $CURRENTPRICE
        sed -i -r 's/["price":]//g' $CURRENTPRICE
}

GetDateTime

CURRENTPRICE=$dir/CurrentPrice.txt
HOURLYFILE=$dir/HourlyStorage.txt
EXCELSPREADSHEET=$dir/Spreadsheet$Year.xls
PASTEXCELSPREADSHEET=$dir/Spreadsheet$PastYear.xls
...
then
	...
	then
		...
		echo "Date	Avg Price	 Mined 24H	Value Mined 24H" >> "$EXCELSPREADSHEET"
	fi
